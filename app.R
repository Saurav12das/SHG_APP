...
precip <- st_read("NE_Precipitaion_Map/NE_normal_precip.shp", quiet = TRUE)
precip <- st_transform(precip, 4326)

# Soil organic matter data (generated by get_wss_soil.R)
soil_om <- st_read("data/ne_soil_om.gpkg", quiet = TRUE)
...
# color palette for soil OM
om_pal <- colorNumeric(palette = "YlOrBr", domain = soil_om$om, na.color = NA)
  ...
  output$map <- renderLeaflet({
    map_base = leaflet() %>%
      ...
      addProviderTiles(providers$Esri.WorldImagery,
                       group = "World Imagery") %>%
      addPolygons(
        data = precip, opacity = 10, fillColor = ~nc_pal(precip_in), stroke = TRUE, color = "black", weight = 0.7,
      ...
      addPolygons(
        data = soil_om, opacity = 0.8, fillColor = ~om_pal(om), stroke = TRUE,
        color = "black", weight = 0.4,
        label = ~paste0("OM: ", round(om, 2), "%"),
        highlight = highlightOptions(weight = 3, color = "red", bringToFront = TRUE),
        group = "Soil OM"
      ) %>%
      addLegend(pal = om_pal, values = soil_om$om, title = "Soil OM (%)",
                position = "bottomleft", group = "Soil OM")
    ...
    map = map_base %>% addCircleMarkers(
      ...
      addCircleMarkers(
        ...
        overlayGroups = c("Soil OM", "Precipitaion","Reference", "Cropland"), options = layersControlOptions(collapsed = F),
      ) %>%
      addSearchOSM() %>%
      addResetMapButton()
  })
